...
// Fetch pending process complaints
$query = "SELECT id, complainant_name, complaint_date, status FROM complaints WHERE status = 'Pending'";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['complaint_id'], $_POST['new_status'])) {
    ...
    // Update the status and automatically set the updated_at timestamp
    $update_query = "UPDATE complaints SET status = '$new_status', updated_at = NOW() WHERE id = $complaint_id";
